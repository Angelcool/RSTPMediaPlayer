...
namespace EasyPlayerNetSDK
{
    public class PlayerSdk
    {
        ...
        public struct EASY_PALYER_OSD
        {
            ...
            public string stOSD;      //OSD字符串
            public uint alpha;        //透明alpha通道，取值0-255，标识为越来越不透明
            public uint color;        //OSD显示文字颜色 RGB(0xf9,0xf9,0xf9)
            public uint shadowcolor;  //OSD显示背景颜色 全为0背景透明
            public tagRECT rect;      //OSD基于图像右上角显示区域
            public int size;		  //OSD显示文本字体大小 just D3D Support
        }
        ...
        public static extern int EasyPlayer_Init(string key= "79393674363469576B5A7541725370636F395572635046515246526655477868655756794C6D56345A6546584446616732504467523246326157346D516D466962334E68514449774D545A4659584E355247467964326C75564756686257566863336B3D");
       ...
       //?EasyPlayer_OpenStream@@YAHPBDPAUHWND__@@W4__RENDER_FORMAT@@H00P6GHHPAHHPADPAURTSP_FRAME_INFO@@@ZPAX_N44M@Z
        ...
        public static int EasyPlayer_ShowOSD(int channelId, int show, Rectangle lpSrcRect, Color color, string stOSD, int size)
        {
            tagRECT rect = new tagRECT { left = (int)lpSrcRect.X, bottom = (int)lpSrcRect.Height, right = (int)lpSrcRect.Width, top = (int)lpSrcRect.Y };
            EASY_PALYER_OSD osd = new EASY_PALYER_OSD { };
            osd.stOSD = stOSD;
            osd.rect = rect;
            osd.size = size;
            osd.alpha = color.A;
            osd.color = (uint)(color.R << 16 | color.G << 8 | color.B);
            osd.shadowcolor = 0;
            return EasyPlayer_ShowOSD(channelId, show, osd);
        ...
        /// <param name="channelId">通道ID，EasyPlayer_OpenStream函数返回值.</param>
        /// <param name="show">0=不显示，1=显示</param>
        /// <param name="osd">详见EASY_PALYER_OSD结构体说明</param>
        /// <returns></returns>
