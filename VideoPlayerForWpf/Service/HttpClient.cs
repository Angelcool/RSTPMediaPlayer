...
namespace VideoPlayerForWpf
{
    ...
    public class HttpClient
    {
        ...
        static HttpClient()
        {
            url = ConfigurationManager.AppSettings.Get("ExternalUrl") ?? "http://127.0.0.1:8099";
        }
        ...
        /// <returns></returns>
        public static HttpResult GetAsync(string strUrl, string param)
        {
            item = new HttpItem()
            {
                URL = string.Format("{0}/{1}?{2}", url, strUrl, param),//URL     必需项 
        ...
        public static HttpResult PostAsync(string strUrl, string postData)
        {
            item = new HttpItem()
            {
                URL = string.Format("{0}/{1}", url, strUrl),//URL     必需项 
        ...
        /// <param name="postData"></param>
        /// <returns></returns>
        public static HttpResult SoapAsync(string strUrl, string postData)
        {
            item = new HttpItem()
            {
                URL = strUrl,//URL     必需项 
                ...
                UserAgent = "Mozilla/5.0 (compatible; MSIE 9.0; Windows NT 6.1; Trident/5.0)",//用户的浏览器类型，版本，操作系统     可选项有默认值  
                Accept = "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8",//    可选项有默认值  
                ContentType = "application/soap+xml; charset=utf-8",//返回类型    可选项有默认值 
                PostEncoding = Encoding.UTF8,
                Encoding = Encoding.UTF8,
                //Referer = referer,//来源URL     可选项  
                //Allowautoredirect = true,//是否根据３０１跳转     可选项  
        ...
        /// <param name="postData"></param>
        /// <returns></returns>
        public static Task<HttpResult> PostDataAsync(string strUrl, string postData)
        {
